...
class TikTokProfileDataFetcher:
    def __init__(self, api_key, tiktok_handle):
        ...
        self.profile_url = f"https://www.tiktok.com/@{self.tiktok_handle}"

    def trigger_data_fetch(self):
        url = f"{self.base_url}/trigger"
        data = json.dumps([{"url": self.profile_url}])
        params = {
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python script.py <tiktok_handle>")
        sys.exit(1)
    api_key = "7e4fe84a-14b3-4be5-b82c-4f2432600c58"
    tiktok_handle = sys.argv[1]
    fetcher = TikTokProfileDataFetcher(api_key, tiktok_handle)
    ...
    if response.status_code == 200:
        ...
        if snapshot_info and 'snapshot_id' in snapshot_info:
            ...
            if snapshot_response.status_code == 200:
                ...
                json_file = f'output/tiktok_{tiktok_handle}/profile.json'
                csv_file = f'output/tiktok_{tiktok_handle}/profile.csv'
